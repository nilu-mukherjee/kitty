...
- [x] Live URL works — https://kitty-1003427733440.asia-south1.run.app deployed; full flow (create/join/add/restriction/split/finalize) verified via curl against prod, via the UI in a real Chrome browser, **and via real WebMCP tool calls in two separate WebMCP-enabled clients**:
  - **Chrome 149+** with `chrome://flags/#enable-webmcp-testing` enabled — via DevTools console, `document.modelContext.getTools()` returned all 7 correctly-scoped tools (descriptions personalized to the joined participant's name), and `document.modelContext.executeTool(tool, JSON.stringify(args))` actually executed `add_item` end-to-end — the order updated with the real item, notes, and price. **Key finding for the demo/any future debugging:** Chrome's native `executeTool` takes the tool object from `getTools()` (not a name string) and wants its arguments JSON-stringified, not passed as a raw object — `executeTool(tool, JSON.stringify(args))`, not `executeTool('name', args)`.
  - **ChatGPT's in-app browser** — same result: all 7 tools discovered correctly-scoped to the joined participant, and `add_item` executed end-to-end against the live deployment.
