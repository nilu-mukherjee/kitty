...
- **Framework:** Next.js, deployed on **Firebase App Hosting** (GCP) — GitHub-connected, auto-detects Next.js, builds/deploys on push. Runs on Cloud Run under the hood, so it gets a live HTTPS URL with minimal setup.
- **State:** One in-memory session object per order (`Map<sessionId, OrderState>`) — **but see the instance-scaling risk below**. Firestore is the fallback if in-memory proves unreliable.
...
> **GCP-specific gotcha:** Firebase App Hosting runs on Cloud Run, which can scale to multiple stateless instances — an in-memory `Map` won't be shared across them. Pin the backend to a **single instance** (`minInstances: 1`, `maxInstances: 1` in `apphosting.yaml`) so the in-memory store stays valid for the hackathon demo. If reliability becomes a problem, swap to **Firestore** for session state (still no server to manage, and it's natively part of the GCP/Firebase stack).
...
| Phase | Time | Output |
|---|---|---|
| 1. Scaffold + deploy skeleton | 1h | Next.js app live on Firebase App Hosting immediately — get a real URL working before building features |
...
- [ ] Live URL works in ChatGPT's in-app browser or Chrome 149+ with `chrome://flags/#enable-webmcp-testing`
...
- **In-memory state on Cloud Run** — if the instance-pinning config in Section 4 is misconfigured (or Firebase App Hosting autoscales anyway), participants could see inconsistent cart state. Test with two simultaneous browser sessions before recording the demo; fall back to Firestore if it's flaky.
